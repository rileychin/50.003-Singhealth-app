...
class TenantViewNoncompliance extends StatefulWidget {
  ...
  @override
  _TenantViewNoncomplianceState createState() => _TenantViewNoncomplianceState(user,firestoreInstance);
}
...
class _TenantViewNoncomplianceState extends State<TenantViewNoncompliance> {
  ...
  Future<void> updateIncidentData() async {
    DocumentSnapshot documentSnapshot = await firestoreInstance.collection('tenant').doc(user.uid).get();
    print(documentSnapshot.data());
    shopName = documentSnapshot.data()['shopName'];
    institution = documentSnapshot.data()['institution'];
    incidentData = await firestoreInstance.collection('institution').doc(institution).collection('tenant')
        .doc(shopName).collection('nonComplianceReport').get();
  }
  ...
  void updateIncidentList() {
    ...
    print(incidentData);
    String inc = '';
    incidentData.docs.forEach((element) {inc += element['incidentName'] + ':';});
