...
class AdminDeleteTenant extends StatefulWidget {
  ...
  AdminDeleteTenant({Key key, this.user, this.admin}) : super(key: key);
  @override
...
class _AdminDeleteTenantState extends State<AdminDeleteTenant> {
  ...
  Widget build(BuildContext context) {
    if (FullTenantList == null)
      return Center(child: CircularProgressIndicator());
    return Scaffold(
        appBar: AppBar(
          backgroundColor: Colors.redAccent,
          title:
              Text('Deleting tenant for institution ${admin['institution']}'),
        ),
        body: Align(
  ...
  void deleteTenant() async {
    if (_shopName == null) {
      Toast.show("Please select a tenant", context,
      ...
      List<String> FnBList =
          Institution.convertToStringList(snapshot.data()['FnBTenantList']);
      List<String> NonFnBList =
          Institution.convertToStringList(snapshot.data()['NonFnBTenantList']);
      ...
      Toast.show("Tenant removed successfully", context,
          duration: Toast.LENGTH_SHORT, gravity: Toast.BOTTOM);
